...
let destination1 = {
  ...
  */
  ...
  let destinationNamesWithin500Kms = travelDestinations.filter(x=>x.distanceKms<=500).map(x=>x.destinationName)
   let destinationNameReachableByFerry =travelDestinations.filter(obj=>obj.transportations.includes("ferry")).map(x=>x.destinationName)
   let destinationNamesMoreThan300KmsAwayByTrain =travelDestinations.filter((obj)=>obj.transportations.includes("train")&&obj.distanceKms>300).map(x=> x.destinationName)
